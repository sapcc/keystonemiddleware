...
LOG = logging.getLogger("designate.domain_resolver_from_token")
CONF = cfg.CONF
...
class DomainResolverFromTokenMiddleware(AuthProtocol):
    ...
    def __init__(self, app, conf):
        ...
        if cache_enabled:
            ...
            LOG.info("[domain_resolver_from_token:init] Cache region initialized "
    ...
    def _get_cached_domain_id(self, domain_name):
        ...
        try:
            ...
            LOG.debug(f"Cache read failed for {domain_name}: {e}")
            return None

    def _set_cached_domain_id(self, domain_name, domain_id):
        if not (self._cache_region and domain_name and domain_id):
            return
        ttl = CONF.get("domain_resolver_cache_ttl", 600)
        try:
            self._cache_region.set(domain_name, domain_id, ttl=ttl)
        except Exception as e:
            LOG.debug(f"Cache write failed for {domain_name}: {e}")

    def process_request(self, request):
        """Executed for each HTTP request before reaching the API controller."""
        result = super(
        ...
        if domain_name:
            ...
            try:
                ...
                LOG.error(f"Failed to resolve domain '{domain_name}'"
                          f" via Keystone: {e}")
        if domain_id:
            request.environ['HTTP_X_DOMAIN_ID'] = domain_id
